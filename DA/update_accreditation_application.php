...
try {
    ...
    if (!$cso_review) {
        ...
        $sql = "UPDATE accreditation_application SET status = ?, remarks = ?, schedule = ?, status_updated_at = NOW() WHERE application_id = ?";
        ...
        if ($cso_status === 'Approved') {
            $status = 'Pending';
            $sql = "UPDATE accreditation_application SET cso_status = ?, cso_status_updated_at = NOW(), status = ?, remarks = ?, schedule = ?, status_updated_at = NOW() WHERE application_id = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) throw new Exception('Database error preparing statement: ' . $conn->error);

            $stmt->bind_param("ssssi", $cso_status, $status, $remarks, $schedule, $application_id);
        } else {
            $sql = "UPDATE accreditation_application SET cso_status = ?, cso_status_updated_at = NOW(), remarks = ?, schedule = ? WHERE application_id = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) throw new Exception('Database error preparing statement: ' . $conn->error);
    ...
    foreach ($documents as $field => $data) {
        ...
        $sql_doc = "UPDATE accreditation_application SET {$status_col} = ?, {$remarks_col} = ?, {$admin_remarks_col} = ? WHERE application_id = ?";
    ...
    $conn->rollback();
    error_log("Update Accreditation Application Error: " . $e->getMessage());
